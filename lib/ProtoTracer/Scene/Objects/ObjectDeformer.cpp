...
bool ObjectDeformer::CheckClipAxis(Vector3D base, bool positive, Axis valueCheckAxis){
...
ObjectDeformer::ObjectDeformer(Object3D* object){
...
ObjectDeformer::ObjectDeformer(Object3D** objects, int objectCount){
    this->objects = objects;
    this->objectCount = objectCount;
}

void ObjectDeformer::PerspectiveDeform(float scaleRatio, Vector3D center, Axis axis){//0.0f close, 1.0f uniform, infinite infinite
...
void ObjectDeformer::SinusoidalDeform(float magnitude, float timeRatio, float periodModifier, float frequencyModifier, Axis axis){
    for(int i = 0; i < objectCount; i++){
        for(int j = 0; j < objects[i]->GetTriangleGroup()->GetVertexCount(); j++){
...
void ObjectDeformer::DropwaveDeform(float magnitude, float timeRatio, float periodModifier, float frequencyModifier, Axis axis){
    for(int i = 0; i < objectCount; i++){
        for(int j = 0; j < objects[i]->GetTriangleGroup()->GetVertexCount(); j++){
            Vector3D base = objects[i]->GetTriangleGroup()->GetVertices()[j];
...
void ObjectDeformer::SineWaveSurfaceDeform(Vector3D offset, float magnitude, float timeRatio, float periodModifier, float frequencyModifier, Axis axis){
    for(int i = 0; i < objectCount; i++){
        for(int j = 0; j < objects[i]->GetTriangleGroup()->GetVertexCount(); j++){
...
void ObjectDeformer::CosineInterpolationDeformer(float* pointMultiplier, int points, float scale, float minAxis, float maxAxis, Axis selectionAxis, Axis deformAxis){
...
void ObjectDeformer::AxisZeroClipping(bool positive, Axis clipAxis, Axis valueCheckAxis){
    for(int i = 0; i < objectCount; i++){
        for(int j = 0; j < objects[i]->GetTriangleGroup()->GetVertexCount(); j++){
            Vector3D base = objects[i]->GetTriangleGroup()->GetVertices()[j];
