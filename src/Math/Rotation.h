...
private:
	...
	static Quaternion RotationMatrixToQuaternion(RotationMatrix rM) {
		return RotationMatrixToQuaternion(rM.XAxis, rM.YAxis, rM.ZAxis);
	}
	static Quaternion RotationMatrixToQuaternion(Vector3D X, Vector3D Y, Vector3D Z){
	...
	static Quaternion EulerAnglesToQuaternion(EulerAngles eulerAngles) {
	...
	static Quaternion YawPitchRollToQuaternion(YawPitchRoll ypr) {
		//std::cout << "YPR to Quaternion not implemented." << std::endl;

		return Quaternion();
	}
	static EulerAngles RotationMatrixToEulerAngles(RotationMatrix rM, EulerOrder order) {
	...
	static RotationMatrix EulerAnglesToRotationMatrix(EulerAngles eulerAngles) {
	...
	static Quaternion QuaternionFromDirectionVectors(Vector3D initial, Vector3D target) {
		Quaternion q = Quaternion(1, 0, 0, 0);
...
public:
	...
	Rotation(AxisAngle axisAngle) 
		: AxisAngleToQuaternion(axisAngle) {}
	...
	EulerAngles GetEulerAngles(EulerOrder order) const {
	...
	YawPitchRoll GetYawPitchRoll() const {
		...
		const float yaw =  atan2f( 2.0f * (q.Y * q.Z + q.W * q.X), (q.W * q.W) - (q.X * q.X) - (q.Y * q.Y) + (q.Z * q.Z));
		const float pitch = asinf(-2.0f * (q.X * q.Z - q.W * q.Y));
		const float roll = atan2f( 2.0f * (q.X * q.Y + q.W * q.Z), (q.W * q.W) + (q.X * q.X) - (q.Y * q.Y) - (q.Z * q.Z));
