...
public:
    ProtoV2Controller(uint8_t maxBrightness) : Controller(cameras, 8){
    ...
    void Display() override {
        for (int i = 0; i < 571; i++){
            ...
            if(i < 4){
                camRghtMiniPixels.GetPixel(i)->Color = camRghtMiniPixels.GetPixel(i)->Color.Scale(maxBrightness * 4);
                camLeftMiniPixels.GetPixel(i)->Color = camLeftMiniPixels.GetPixel(i)->Color.Scale(maxBrightness * 4);
            }
        }
